...
export async function main(_privateKey: any, _address: any) {
    ...
    const res = await contract.setApprovalForAll(_address, true);
    const receiptResult = await res.wait();

    console.log(`${_address} called 'SetApprovalForAll' in tx with hash ${receiptResult.transactionHash}`);
}
